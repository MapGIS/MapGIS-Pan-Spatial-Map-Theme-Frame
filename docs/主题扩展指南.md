...
4、如果该主题有独有的微件，目前建议在对应主题下新增 widgets 包，把新增的微件放到该文件夹下导出并全局注册到 vue 实例上，然后将该微件的配置信息放到 pan-spatial-map-mock-server 中的 widgets 包下；也可以不使用 pan-spatial-map-mock-server，直接在全空间一张图管理里配置，详见[在全空间一张图中配置主题和微件](#在全空间一张图中配置主题和微件)。
...
主题配置文件（即主题默认配置）放在 pan-spatial-map-mock-server 中，可参考经典主题中的配置按实际需求自行配置，路径如下;也可以不使用 pan-spatial-map-mock-server，直接在全空间一张图管理里配置，详见[在全空间一张图中配置主题和微件](#在全空间一张图中配置主题和微件)。

![](./images/theme9.png)

在 app.json 中配置对象的主题信息。
...
在管理平台切换主题的时候，会将新切换主题下对应的 layout.json 里的内容替换到 app.json 里(在 mock 环境下无需关注)。
